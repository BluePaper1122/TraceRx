...
Yiyi, the source is ready for a synthetic-demo deployment after GitHub checks pass. No new hosted URL has been verified in this task.
...
6. Configure a session key through AI connection, run one development image, inspect quotes and fields, then save a small measured development report. Freeze extraction choices before the held-out test run. Record actual model, sample counts and failures. Never describe fixture replay or parser scores as live accuracy.
7. Rehearse from another device with no local server. If deployment fails, inspect build logs and reproduce the failing dependency/runtime. For rollback, revert the specific release commit after preserving any subsequent work; never force-push or reset user changes.
